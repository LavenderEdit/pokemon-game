...
class Player:
    ...
    def load_sprites(self):
        """Loads specific coordinates from the Atlas using the SpriteSheet utility."""
        sheet = SpriteSheet(PLAYER_SPRITE_PATH)
        sprites = {}
        
        try:
            sprites = {
                DIR_DOWN: sheet.images_at(PLAYER_RECTS[DIR_DOWN], colorkey=-1),
                DIR_UP: sheet.images_at(PLAYER_RECTS[DIR_UP], colorkey=-1),
                DIR_LEFT: sheet.images_at(PLAYER_RECTS[DIR_LEFT], colorkey=-1),
                DIR_RIGHT: sheet.images_at(PLAYER_RECTS[DIR_RIGHT], colorkey=-1)
            }
    ...
    def update(self):
        """Updates animation frames based on movement state."""
        self.handle_input()
